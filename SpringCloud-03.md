...
2. 给应用"segmentfault"创建三个环境的配置文件
...
调整后，本地的<http://localhost:8080/user> 的内容变化:
...
服务端修改配置之后，客户端需要refresh手动刷新一下上下文才能更新配置

客户端可以配置定时任务，定时刷新上下文，ContextRefresher.refresh();不会调整文件内容，调整内存内容
